# openapi-client
No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

This Python package is automatically generated by the [OpenAPI Generator](https://openapi-generator.tech) project:

- API version: 0.1.0
- Package version: 1.0.0
- Generator version: 7.6.0
- Build package: org.openapitools.codegen.languages.PythonClientCodegen

## Requirements.

Python 3.7+

## Installation & Usage
### pip install

If the python package is hosted on a repository, you can install directly using:

```sh
pip install git+https://github.com/GIT_USER_ID/GIT_REPO_ID.git
```
(you may need to run `pip` with root permission: `sudo pip install git+https://github.com/GIT_USER_ID/GIT_REPO_ID.git`)

Then import the package:
```python
import openapi_client
```

### Setuptools

Install via [Setuptools](http://pypi.python.org/pypi/setuptools).

```sh
python setup.py install --user
```
(or `sudo python setup.py install` to install the package for all users)

Then import the package:
```python
import openapi_client
```

### Tests

Execute `pytest` to run the tests.

## Getting Started

Please follow the [installation procedure](#installation--usage) and then run the following:

```python

import openapi_client
from openapi_client.rest import ApiException
from pprint import pprint

# Defining the host is optional and defaults to https://api.data.igvf.org
# See configuration.py for a list of all supported configuration parameters.
configuration = openapi_client.Configuration(
    host = "https://api.data.igvf.org"
)



# Enter a context with an instance of the API client
with openapi_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = openapi_client.DefaultApi(api_client)
    debug = True # bool | Enables debug mode for the search. (optional)
    var_field = ['var_field_example'] # List[str] | Fields to include in the response. Can be repeated for multiple fields. (optional)
    frame = 'frame_example' # str | Specifies the format of the returned objects. (optional)
    var_from = 56 # int | Starting index for pagination. (optional)
    limit = 'limit_example' # str | Maximum number of results to return. Use 'all' for all results. (optional)
    mode = 'mode_example' # str | Specifies the search mode. (optional)
    sort = ['sort_example'] # List[str] | Fields to sort results by. Prefix with '-' for descending order. Can be repeated for multiple sort fields. (optional)
    type = ['type_example'] # List[str] | Type of objects to return. Can be repeated for multiple types. (optional)
    config = 'config_example' # str | Configuration options for the search. (optional)
    advanced_query = 'advanced_query_example' # str | Advanced query string for complex searches. (optional)
    query = 'query_example' # str | Query string for searching. (optional)
    field_filters = None # object | Any field from any object type can be used as a filter. Use '!=' for negation, '*' as a wildcard, and 'lt:', 'lte:', 'gt:', 'gte:' for range queries on numeric fields. (optional)

    try:
        # Search for objects in the IGVF Project
        api_response = api_instance.search(debug=debug, var_field=var_field, frame=frame, var_from=var_from, limit=limit, mode=mode, sort=sort, type=type, config=config, advanced_query=advanced_query, query=query, field_filters=field_filters)
        print("The response of DefaultApi->search:\n")
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling DefaultApi->search: %s\n" % e)

```

## Documentation for API Endpoints

All URIs are relative to *https://api.data.igvf.org*

Class | Method | HTTP request | Description
------------ | ------------- | ------------- | -------------
*DefaultApi* | [**search**](docs/DefaultApi.md#search) | **GET** /search | Search for objects in the IGVF Project


## Documentation For Models

 - [AccessKey](docs/AccessKey.md)
 - [AccessKeySubmittedBy](docs/AccessKeySubmittedBy.md)
 - [AlignmentFile](docs/AlignmentFile.md)
 - [AlignmentFileDerivedFromInner](docs/AlignmentFileDerivedFromInner.md)
 - [AlignmentFileFileSet](docs/AlignmentFileFileSet.md)
 - [AlignmentFileReferenceFilesInner](docs/AlignmentFileReferenceFilesInner.md)
 - [AnalysisSet](docs/AnalysisSet.md)
 - [AnalysisSetDonorsInner](docs/AnalysisSetDonorsInner.md)
 - [AnalysisSetSamplesInner](docs/AnalysisSetSamplesInner.md)
 - [AnalysisStep](docs/AnalysisStep.md)
 - [AnalysisStepAward](docs/AnalysisStepAward.md)
 - [AnalysisStepLab](docs/AnalysisStepLab.md)
 - [AnalysisStepParentsInner](docs/AnalysisStepParentsInner.md)
 - [AnalysisStepWorkflow](docs/AnalysisStepWorkflow.md)
 - [AssayTerm](docs/AssayTerm.md)
 - [AssayTermIsAInner](docs/AssayTermIsAInner.md)
 - [AssociatedPhenotypesInner](docs/AssociatedPhenotypesInner.md)
 - [Attachment](docs/Attachment.md)
 - [Attachment1](docs/Attachment1.md)
 - [AuxiliarySet](docs/AuxiliarySet.md)
 - [Award](docs/Award.md)
 - [Biomarker](docs/Biomarker.md)
 - [BiomarkerGene](docs/BiomarkerGene.md)
 - [ConfigurationFile](docs/ConfigurationFile.md)
 - [ConfigurationFileSeqspecOfInner](docs/ConfigurationFileSeqspecOfInner.md)
 - [ConstructLibrarySet](docs/ConstructLibrarySet.md)
 - [ConstructLibrarySetLargeScaleLociList](docs/ConstructLibrarySetLargeScaleLociList.md)
 - [ContentType](docs/ContentType.md)
 - [ContentType1](docs/ContentType1.md)
 - [CrisprModification](docs/CrisprModification.md)
 - [CuratedSet](docs/CuratedSet.md)
 - [Document](docs/Document.md)
 - [DocumentAward](docs/DocumentAward.md)
 - [DocumentLab](docs/DocumentLab.md)
 - [DocumentSubmittedBy](docs/DocumentSubmittedBy.md)
 - [DocumentsInner](docs/DocumentsInner.md)
 - [Gene](docs/Gene.md)
 - [GeneLocation](docs/GeneLocation.md)
 - [GenomeBrowserAnnotationFile](docs/GenomeBrowserAnnotationFile.md)
 - [HumanDonor](docs/HumanDonor.md)
 - [Image](docs/Image.md)
 - [ImageFile](docs/ImageFile.md)
 - [InVitroSystem](docs/InVitroSystem.md)
 - [InVitroSystemBiomarkersInner](docs/InVitroSystemBiomarkersInner.md)
 - [InVitroSystemConstructLibrarySetsInner](docs/InVitroSystemConstructLibrarySetsInner.md)
 - [InVitroSystemDemultiplexedFrom](docs/InVitroSystemDemultiplexedFrom.md)
 - [InVitroSystemModificationsInner](docs/InVitroSystemModificationsInner.md)
 - [InVitroSystemPooledFromInner](docs/InVitroSystemPooledFromInner.md)
 - [InVitroSystemSampleTermsInner](docs/InVitroSystemSampleTermsInner.md)
 - [InVitroSystemTreatmentsInner](docs/InVitroSystemTreatmentsInner.md)
 - [InputContentType](docs/InputContentType.md)
 - [InputContentTypeAnyOf](docs/InputContentTypeAnyOf.md)
 - [InstitutionalCertificate](docs/InstitutionalCertificate.md)
 - [IntegratedContentFilesInner](docs/IntegratedContentFilesInner.md)
 - [Lab](docs/Lab.md)
 - [ListOfOpenReadingFramesORFInner](docs/ListOfOpenReadingFramesORFInner.md)
 - [Locus](docs/Locus.md)
 - [MatrixFile](docs/MatrixFile.md)
 - [MeasurementSet](docs/MeasurementSet.md)
 - [MeasurementSetAssayTerm](docs/MeasurementSetAssayTerm.md)
 - [MeasurementSetAuxiliarySetsInner](docs/MeasurementSetAuxiliarySetsInner.md)
 - [MeasurementSetRelatedMultiomeDatasetsInner](docs/MeasurementSetRelatedMultiomeDatasetsInner.md)
 - [ModelFile](docs/ModelFile.md)
 - [ModelSet](docs/ModelSet.md)
 - [ModelSetSoftwareVersion](docs/ModelSetSoftwareVersion.md)
 - [Modification](docs/Modification.md)
 - [MultiplexedSample](docs/MultiplexedSample.md)
 - [MultiplexedSampleBarcodeSampleMap](docs/MultiplexedSampleBarcodeSampleMap.md)
 - [OpenReadingFrame](docs/OpenReadingFrame.md)
 - [OutputContentType](docs/OutputContentType.md)
 - [Page](docs/Page.md)
 - [PageLayout](docs/PageLayout.md)
 - [PageLayoutComponents](docs/PageLayoutComponents.md)
 - [PageParent](docs/PageParent.md)
 - [PhenotypeTerm](docs/PhenotypeTerm.md)
 - [PhenotypicFeature](docs/PhenotypicFeature.md)
 - [PhenotypicFeatureFeature](docs/PhenotypicFeatureFeature.md)
 - [PhenotypicFeaturesInner](docs/PhenotypicFeaturesInner.md)
 - [PlatformTerm](docs/PlatformTerm.md)
 - [PredictionSet](docs/PredictionSet.md)
 - [PrimaryCell](docs/PrimaryCell.md)
 - [PrimaryCellPartOf](docs/PrimaryCellPartOf.md)
 - [PrimaryCellPooledFromInner](docs/PrimaryCellPooledFromInner.md)
 - [Publication](docs/Publication.md)
 - [ReferenceFile](docs/ReferenceFile.md)
 - [RelatedDonor](docs/RelatedDonor.md)
 - [RelatedDonorDonor](docs/RelatedDonorDonor.md)
 - [RodentDonor](docs/RodentDonor.md)
 - [RodentDonorDocumentsInner](docs/RodentDonorDocumentsInner.md)
 - [RodentDonorPhenotypicFeaturesInner](docs/RodentDonorPhenotypicFeaturesInner.md)
 - [RodentDonorSourcesInner](docs/RodentDonorSourcesInner.md)
 - [SampleTerm](docs/SampleTerm.md)
 - [Search200Response](docs/Search200Response.md)
 - [Search200ResponseFacetsInner](docs/Search200ResponseFacetsInner.md)
 - [Search200ResponseFacetsInnerTermsInner](docs/Search200ResponseFacetsInnerTermsInner.md)
 - [Search200ResponseGraphInner](docs/Search200ResponseGraphInner.md)
 - [SequenceFile](docs/SequenceFile.md)
 - [SequenceFileSequencingPlatform](docs/SequenceFileSequencingPlatform.md)
 - [SignalFile](docs/SignalFile.md)
 - [SmallScaleGeneListInner](docs/SmallScaleGeneListInner.md)
 - [Software](docs/Software.md)
 - [SoftwareVersion](docs/SoftwareVersion.md)
 - [Source](docs/Source.md)
 - [SourcesInner](docs/SourcesInner.md)
 - [TabularFile](docs/TabularFile.md)
 - [TechnicalSample](docs/TechnicalSample.md)
 - [Tile](docs/Tile.md)
 - [Tissue](docs/Tissue.md)
 - [TissuePartOf](docs/TissuePartOf.md)
 - [TissuePooledFromInner](docs/TissuePooledFromInner.md)
 - [Treatment](docs/Treatment.md)
 - [User](docs/User.md)
 - [WholeOrganism](docs/WholeOrganism.md)
 - [Workflow](docs/Workflow.md)
 - [WorkflowStandardsPage](docs/WorkflowStandardsPage.md)


<a id="documentation-for-authorization"></a>
## Documentation For Authorization

Endpoints do not require authorization.


## Author




