#' Create a new OpenReadingFrameResults
#'
#' @description
#' OpenReadingFrameResults Class
#'
#' @docType class
#' @title OpenReadingFrameResults
#' @description OpenReadingFrameResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{OpenReadingFrame}) [optional]
...
#' @export
OpenReadingFrameResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new OpenReadingFrameResults class.
    #'
    #' @description
    #' Initialize a new OpenReadingFrameResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return OpenReadingFrameResults in JSON format
    #' @export
    toJSON = function() {
      OpenReadingFrameResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        OpenReadingFrameResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        OpenReadingFrameResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        OpenReadingFrameResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        OpenReadingFrameResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        OpenReadingFrameResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      OpenReadingFrameResultsObject
    },
    #' Deserialize JSON string into an instance of OpenReadingFrameResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of OpenReadingFrameResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of OpenReadingFrameResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[OpenReadingFrame]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to OpenReadingFrameResults
    #'
    #' @description
    #' Validate JSON input with respect to OpenReadingFrameResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of OpenReadingFrameResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# OpenReadingFrameResults$unlock()
#
## Below is an example to define the print function
# OpenReadingFrameResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# OpenReadingFrameResults$lock()
