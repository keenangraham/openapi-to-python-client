#' Create a new AssayTermResults
#'
#' @description
#' AssayTermResults Class
#'
#' @docType class
#' @title AssayTermResults
#' @description AssayTermResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{AssayTerm}) [optional]
...
AssayTermResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new AssayTermResults class.
    #'
    #' @description
    #' Initialize a new AssayTermResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return AssayTermResults in JSON format
    #' @export
    toJSON = function() {
      AssayTermResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        AssayTermResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        AssayTermResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        AssayTermResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        AssayTermResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        AssayTermResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      AssayTermResultsObject
    },
    #' Deserialize JSON string into an instance of AssayTermResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of AssayTermResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of AssayTermResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[AssayTerm]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to AssayTermResults
    #'
    #' @description
    #' Validate JSON input with respect to AssayTermResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of AssayTermResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# AssayTermResults$unlock()
#
## Below is an example to define the print function
# AssayTermResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AssayTermResults$lock()
