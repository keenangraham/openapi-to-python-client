#' Create a new PhenotypicFeatureResults
#'
#' @description
#' PhenotypicFeatureResults Class
#'
#' @docType class
#' @title PhenotypicFeatureResults
#' @description PhenotypicFeatureResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{PhenotypicFeature}) [optional]
...
#' @export
PhenotypicFeatureResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PhenotypicFeatureResults class.
    #'
    #' @description
    #' Initialize a new PhenotypicFeatureResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PhenotypicFeatureResults in JSON format
    #' @export
    toJSON = function() {
      PhenotypicFeatureResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PhenotypicFeatureResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PhenotypicFeatureResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PhenotypicFeatureResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PhenotypicFeatureResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PhenotypicFeatureResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PhenotypicFeatureResultsObject
    },
    #' Deserialize JSON string into an instance of PhenotypicFeatureResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PhenotypicFeatureResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PhenotypicFeatureResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[PhenotypicFeature]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PhenotypicFeatureResults
    #'
    #' @description
    #' Validate JSON input with respect to PhenotypicFeatureResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PhenotypicFeatureResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PhenotypicFeatureResults$unlock()
#
## Below is an example to define the print function
# PhenotypicFeatureResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PhenotypicFeatureResults$lock()
