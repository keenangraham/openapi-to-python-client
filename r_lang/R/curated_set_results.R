#' Create a new CuratedSetResults
#'
#' @description
#' CuratedSetResults Class
#'
#' @docType class
#' @title CuratedSetResults
#' @description CuratedSetResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{CuratedSet}) [optional]
...
CuratedSetResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new CuratedSetResults class.
    #'
    #' @description
    #' Initialize a new CuratedSetResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return CuratedSetResults in JSON format
    #' @export
    toJSON = function() {
      CuratedSetResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        CuratedSetResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        CuratedSetResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        CuratedSetResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        CuratedSetResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        CuratedSetResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      CuratedSetResultsObject
    },
    #' Deserialize JSON string into an instance of CuratedSetResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of CuratedSetResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of CuratedSetResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[CuratedSet]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to CuratedSetResults
    #'
    #' @description
    #' Validate JSON input with respect to CuratedSetResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of CuratedSetResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# CuratedSetResults$unlock()
#
## Below is an example to define the print function
# CuratedSetResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CuratedSetResults$lock()
