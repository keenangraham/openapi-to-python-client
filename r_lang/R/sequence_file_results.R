#' Create a new SequenceFileResults
#'
#' @description
#' SequenceFileResults Class
#'
#' @docType class
#' @title SequenceFileResults
#' @description SequenceFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{SequenceFile}) [optional]
...
#' @export
SequenceFileResults <- R6::R6Class(
  "SequenceFileResults",
  public = list(
    `@graph` = NULL,
    `@id` = NULL,
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SequenceFileResults class.
    #'
    #' @description
    #' Initialize a new SequenceFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SequenceFileResults in JSON format
    #' @export
    toJSON = function() {
      SequenceFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SequenceFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SequenceFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SequenceFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SequenceFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SequenceFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SequenceFileResultsObject
    },
    #' Deserialize JSON string into an instance of SequenceFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SequenceFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SequenceFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[SequenceFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SequenceFileResults
    #'
    #' @description
    #' Validate JSON input with respect to SequenceFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SequenceFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SequenceFileResults$unlock()
#
## Below is an example to define the print function
# SequenceFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# SequenceFileResults$lock()
