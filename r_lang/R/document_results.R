#' Create a new DocumentResults
#'
#' @description
#' DocumentResults Class
#'
#' @docType class
#' @title DocumentResults
#' @description DocumentResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Document}) [optional]
...
DocumentResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new DocumentResults class.
    #'
    #' @description
    #' Initialize a new DocumentResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return DocumentResults in JSON format
    #' @export
    toJSON = function() {
      DocumentResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        DocumentResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        DocumentResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        DocumentResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        DocumentResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        DocumentResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      DocumentResultsObject
    },
    #' Deserialize JSON string into an instance of DocumentResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of DocumentResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of DocumentResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Document]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to DocumentResults
    #'
    #' @description
    #' Validate JSON input with respect to DocumentResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of DocumentResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# DocumentResults$unlock()
#
## Below is an example to define the print function
# DocumentResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# DocumentResults$lock()
