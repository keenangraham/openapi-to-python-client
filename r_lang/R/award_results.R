...
#' @docType class
#' @title AwardResults
#' @description AwardResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Award}) [optional]
...
AwardResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new AwardResults class.
    #'
    #' @description
    #' Initialize a new AwardResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return AwardResults in JSON format
    #' @export
    toJSON = function() {
      AwardResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        AwardResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        AwardResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        AwardResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        AwardResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        AwardResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      AwardResultsObject
    },
    #' Deserialize JSON string into an instance of AwardResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of AwardResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of AwardResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Award]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to AwardResults
    #'
    #' @description
    #' Validate JSON input with respect to AwardResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of AwardResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# AwardResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
