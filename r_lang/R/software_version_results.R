#' Create a new SoftwareVersionResults
#'
#' @description
#' SoftwareVersionResults Class
#'
#' @docType class
#' @title SoftwareVersionResults
#' @description SoftwareVersionResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{SoftwareVersion}) [optional]
...
#' @export
SoftwareVersionResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SoftwareVersionResults class.
    #'
    #' @description
    #' Initialize a new SoftwareVersionResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SoftwareVersionResults in JSON format
    #' @export
    toJSON = function() {
      SoftwareVersionResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SoftwareVersionResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SoftwareVersionResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SoftwareVersionResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SoftwareVersionResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SoftwareVersionResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SoftwareVersionResultsObject
    },
    #' Deserialize JSON string into an instance of SoftwareVersionResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SoftwareVersionResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SoftwareVersionResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[SoftwareVersion]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SoftwareVersionResults
    #'
    #' @description
    #' Validate JSON input with respect to SoftwareVersionResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SoftwareVersionResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SoftwareVersionResults$unlock()
#
## Below is an example to define the print function
# SoftwareVersionResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# SoftwareVersionResults$lock()
