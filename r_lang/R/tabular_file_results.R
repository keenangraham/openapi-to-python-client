#' Create a new TabularFileResults
#'
#' @description
#' TabularFileResults Class
#'
#' @docType class
#' @title TabularFileResults
#' @description TabularFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{TabularFile}) [optional]
...
TabularFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new TabularFileResults class.
    #'
    #' @description
    #' Initialize a new TabularFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return TabularFileResults in JSON format
    #' @export
    toJSON = function() {
      TabularFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        TabularFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        TabularFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        TabularFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        TabularFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        TabularFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      TabularFileResultsObject
    },
    #' Deserialize JSON string into an instance of TabularFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of TabularFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of TabularFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[TabularFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to TabularFileResults
    #'
    #' @description
    #' Validate JSON input with respect to TabularFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of TabularFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# TabularFileResults$unlock()
#
## Below is an example to define the print function
# TabularFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# TabularFileResults$lock()
