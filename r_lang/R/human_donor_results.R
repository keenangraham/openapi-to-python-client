#' Create a new HumanDonorResults
#'
#' @description
#' HumanDonorResults Class
#'
#' @docType class
#' @title HumanDonorResults
#' @description HumanDonorResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{HumanDonor}) [optional]
...
HumanDonorResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new HumanDonorResults class.
    #'
    #' @description
    #' Initialize a new HumanDonorResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return HumanDonorResults in JSON format
    #' @export
    toJSON = function() {
      HumanDonorResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        HumanDonorResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        HumanDonorResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        HumanDonorResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        HumanDonorResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        HumanDonorResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      HumanDonorResultsObject
    },
    #' Deserialize JSON string into an instance of HumanDonorResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of HumanDonorResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of HumanDonorResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[HumanDonor]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to HumanDonorResults
    #'
    #' @description
    #' Validate JSON input with respect to HumanDonorResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of HumanDonorResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# HumanDonorResults$unlock()
#
## Below is an example to define the print function
# HumanDonorResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# HumanDonorResults$lock()
