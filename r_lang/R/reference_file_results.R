#' Create a new ReferenceFileResults
#'
#' @description
#' ReferenceFileResults Class
#'
#' @docType class
#' @title ReferenceFileResults
#' @description ReferenceFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{ReferenceFile}) [optional]
...
#' @export
ReferenceFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new ReferenceFileResults class.
    #'
    #' @description
    #' Initialize a new ReferenceFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return ReferenceFileResults in JSON format
    #' @export
    toJSON = function() {
      ReferenceFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        ReferenceFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        ReferenceFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        ReferenceFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        ReferenceFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        ReferenceFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      ReferenceFileResultsObject
    },
    #' Deserialize JSON string into an instance of ReferenceFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of ReferenceFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of ReferenceFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[ReferenceFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to ReferenceFileResults
    #'
    #' @description
    #' Validate JSON input with respect to ReferenceFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ReferenceFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ReferenceFileResults$unlock()
#
## Below is an example to define the print function
# ReferenceFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ReferenceFileResults$lock()
