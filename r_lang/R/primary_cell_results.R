#' Create a new PrimaryCellResults
#'
#' @description
#' PrimaryCellResults Class
#'
#' @docType class
#' @title PrimaryCellResults
#' @description PrimaryCellResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{PrimaryCell}) [optional]
...
PrimaryCellResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PrimaryCellResults class.
    #'
    #' @description
    #' Initialize a new PrimaryCellResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PrimaryCellResults in JSON format
    #' @export
    toJSON = function() {
      PrimaryCellResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PrimaryCellResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PrimaryCellResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PrimaryCellResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PrimaryCellResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PrimaryCellResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PrimaryCellResultsObject
    },
    #' Deserialize JSON string into an instance of PrimaryCellResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PrimaryCellResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PrimaryCellResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[PrimaryCell]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PrimaryCellResults
    #'
    #' @description
    #' Validate JSON input with respect to PrimaryCellResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PrimaryCellResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PrimaryCellResults$unlock()
#
## Below is an example to define the print function
# PrimaryCellResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PrimaryCellResults$lock()
