#' Create a new PlatformTermResults
#'
#' @description
#' PlatformTermResults Class
#'
#' @docType class
#' @title PlatformTermResults
#' @description PlatformTermResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{PlatformTerm}) [optional]
...
#' @export
PlatformTermResults <- R6::R6Class(
  "PlatformTermResults",
  public = list(
    `@graph` = NULL,
    `@id` = NULL,
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PlatformTermResults class.
    #'
    #' @description
    #' Initialize a new PlatformTermResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PlatformTermResults in JSON format
    #' @export
    toJSON = function() {
      PlatformTermResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PlatformTermResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PlatformTermResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PlatformTermResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PlatformTermResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PlatformTermResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PlatformTermResultsObject
    },
    #' Deserialize JSON string into an instance of PlatformTermResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PlatformTermResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PlatformTermResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[PlatformTerm]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PlatformTermResults
    #'
    #' @description
    #' Validate JSON input with respect to PlatformTermResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PlatformTermResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PlatformTermResults$unlock()
#
## Below is an example to define the print function
# PlatformTermResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PlatformTermResults$lock()
