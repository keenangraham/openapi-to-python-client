#' Create a new MultiplexedSampleResults
#'
#' @description
#' MultiplexedSampleResults Class
#'
#' @docType class
#' @title MultiplexedSampleResults
#' @description MultiplexedSampleResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{MultiplexedSample}) [optional]
...
#' @export
MultiplexedSampleResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new MultiplexedSampleResults class.
    #'
    #' @description
    #' Initialize a new MultiplexedSampleResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return MultiplexedSampleResults in JSON format
    #' @export
    toJSON = function() {
      MultiplexedSampleResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        MultiplexedSampleResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        MultiplexedSampleResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        MultiplexedSampleResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        MultiplexedSampleResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        MultiplexedSampleResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      MultiplexedSampleResultsObject
    },
    #' Deserialize JSON string into an instance of MultiplexedSampleResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of MultiplexedSampleResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of MultiplexedSampleResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[MultiplexedSample]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to MultiplexedSampleResults
    #'
    #' @description
    #' Validate JSON input with respect to MultiplexedSampleResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of MultiplexedSampleResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# MultiplexedSampleResults$unlock()
#
## Below is an example to define the print function
# MultiplexedSampleResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MultiplexedSampleResults$lock()
