...
#' @docType class
#' @title PageResults
#' @description PageResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Page}) [optional]
...
PageResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PageResults class.
    #'
    #' @description
    #' Initialize a new PageResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PageResults in JSON format
    #' @export
    toJSON = function() {
      PageResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PageResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PageResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PageResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PageResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PageResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PageResultsObject
    },
    #' Deserialize JSON string into an instance of PageResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PageResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PageResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Page]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PageResults
    #'
    #' @description
    #' Validate JSON input with respect to PageResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PageResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PageResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
