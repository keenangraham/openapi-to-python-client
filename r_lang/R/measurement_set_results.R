#' Create a new MeasurementSetResults
#'
#' @description
#' MeasurementSetResults Class
#'
#' @docType class
#' @title MeasurementSetResults
#' @description MeasurementSetResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{MeasurementSet}) [optional]
...
#' @export
MeasurementSetResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new MeasurementSetResults class.
    #'
    #' @description
    #' Initialize a new MeasurementSetResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return MeasurementSetResults in JSON format
    #' @export
    toJSON = function() {
      MeasurementSetResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        MeasurementSetResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        MeasurementSetResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        MeasurementSetResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        MeasurementSetResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        MeasurementSetResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      MeasurementSetResultsObject
    },
    #' Deserialize JSON string into an instance of MeasurementSetResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of MeasurementSetResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of MeasurementSetResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[MeasurementSet]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to MeasurementSetResults
    #'
    #' @description
    #' Validate JSON input with respect to MeasurementSetResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of MeasurementSetResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# MeasurementSetResults$unlock()
#
## Below is an example to define the print function
# MeasurementSetResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MeasurementSetResults$lock()
