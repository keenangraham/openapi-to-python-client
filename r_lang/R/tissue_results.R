...
#' @docType class
#' @title TissueResults
#' @description TissueResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Tissue}) [optional]
...
TissueResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new TissueResults class.
    #'
    #' @description
    #' Initialize a new TissueResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return TissueResults in JSON format
    #' @export
    toJSON = function() {
      TissueResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        TissueResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        TissueResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        TissueResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        TissueResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        TissueResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      TissueResultsObject
    },
    #' Deserialize JSON string into an instance of TissueResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of TissueResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of TissueResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Tissue]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to TissueResults
    #'
    #' @description
    #' Validate JSON input with respect to TissueResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of TissueResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# TissueResults$unlock()
#
## Below is an example to define the print function
# TissueResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
