#' Create a new AlignmentFileResults
#'
#' @description
#' AlignmentFileResults Class
#'
#' @docType class
#' @title AlignmentFileResults
#' @description AlignmentFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{AlignmentFile}) [optional]
...
#' @export
AlignmentFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new AlignmentFileResults class.
    #'
    #' @description
    #' Initialize a new AlignmentFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return AlignmentFileResults in JSON format
    #' @export
    toJSON = function() {
      AlignmentFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        AlignmentFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        AlignmentFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        AlignmentFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        AlignmentFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        AlignmentFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      AlignmentFileResultsObject
    },
    #' Deserialize JSON string into an instance of AlignmentFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of AlignmentFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of AlignmentFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[AlignmentFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to AlignmentFileResults
    #'
    #' @description
    #' Validate JSON input with respect to AlignmentFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of AlignmentFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# AlignmentFileResults$unlock()
#
## Below is an example to define the print function
# AlignmentFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AlignmentFileResults$lock()
