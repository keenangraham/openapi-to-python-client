#' Create a new AccessKeyResults
#'
#' @description
#' AccessKeyResults Class
#'
#' @docType class
#' @title AccessKeyResults
#' @description AccessKeyResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{AccessKey}) [optional]
...
AccessKeyResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new AccessKeyResults class.
    #'
    #' @description
    #' Initialize a new AccessKeyResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return AccessKeyResults in JSON format
    #' @export
    toJSON = function() {
      AccessKeyResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        AccessKeyResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        AccessKeyResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        AccessKeyResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        AccessKeyResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        AccessKeyResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      AccessKeyResultsObject
    },
    #' Deserialize JSON string into an instance of AccessKeyResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of AccessKeyResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of AccessKeyResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[AccessKey]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to AccessKeyResults
    #'
    #' @description
    #' Validate JSON input with respect to AccessKeyResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of AccessKeyResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# AccessKeyResults$unlock()
#
## Below is an example to define the print function
# AccessKeyResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AccessKeyResults$lock()
