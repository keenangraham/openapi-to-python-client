#' Create a new SampleTermResults
#'
#' @description
#' SampleTermResults Class
#'
#' @docType class
#' @title SampleTermResults
#' @description SampleTermResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{SampleTerm}) [optional]
...
SampleTermResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SampleTermResults class.
    #'
    #' @description
    #' Initialize a new SampleTermResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SampleTermResults in JSON format
    #' @export
    toJSON = function() {
      SampleTermResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SampleTermResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SampleTermResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SampleTermResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SampleTermResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SampleTermResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SampleTermResultsObject
    },
    #' Deserialize JSON string into an instance of SampleTermResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SampleTermResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SampleTermResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[SampleTerm]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SampleTermResults
    #'
    #' @description
    #' Validate JSON input with respect to SampleTermResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SampleTermResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SampleTermResults$unlock()
#
## Below is an example to define the print function
# SampleTermResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# SampleTermResults$lock()
