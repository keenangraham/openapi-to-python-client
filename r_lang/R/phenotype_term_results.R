#' Create a new PhenotypeTermResults
#'
#' @description
#' PhenotypeTermResults Class
#'
#' @docType class
#' @title PhenotypeTermResults
#' @description PhenotypeTermResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{PhenotypeTerm}) [optional]
...
#' @export
PhenotypeTermResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PhenotypeTermResults class.
    #'
    #' @description
    #' Initialize a new PhenotypeTermResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PhenotypeTermResults in JSON format
    #' @export
    toJSON = function() {
      PhenotypeTermResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PhenotypeTermResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PhenotypeTermResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PhenotypeTermResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PhenotypeTermResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PhenotypeTermResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PhenotypeTermResultsObject
    },
    #' Deserialize JSON string into an instance of PhenotypeTermResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PhenotypeTermResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PhenotypeTermResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[PhenotypeTerm]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PhenotypeTermResults
    #'
    #' @description
    #' Validate JSON input with respect to PhenotypeTermResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PhenotypeTermResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PhenotypeTermResults$unlock()
#
## Below is an example to define the print function
# PhenotypeTermResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PhenotypeTermResults$lock()
