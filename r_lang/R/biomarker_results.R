#' Create a new BiomarkerResults
#'
#' @description
#' BiomarkerResults Class
#'
#' @docType class
#' @title BiomarkerResults
#' @description BiomarkerResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Biomarker}) [optional]
...
BiomarkerResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new BiomarkerResults class.
    #'
    #' @description
    #' Initialize a new BiomarkerResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return BiomarkerResults in JSON format
    #' @export
    toJSON = function() {
      BiomarkerResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        BiomarkerResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        BiomarkerResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        BiomarkerResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        BiomarkerResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        BiomarkerResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      BiomarkerResultsObject
    },
    #' Deserialize JSON string into an instance of BiomarkerResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of BiomarkerResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of BiomarkerResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Biomarker]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to BiomarkerResults
    #'
    #' @description
    #' Validate JSON input with respect to BiomarkerResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of BiomarkerResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# BiomarkerResults$unlock()
#
## Below is an example to define the print function
# BiomarkerResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# BiomarkerResults$lock()
