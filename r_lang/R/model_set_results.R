#' Create a new ModelSetResults
#'
#' @description
#' ModelSetResults Class
#'
#' @docType class
#' @title ModelSetResults
#' @description ModelSetResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{ModelSet}) [optional]
...
ModelSetResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new ModelSetResults class.
    #'
    #' @description
    #' Initialize a new ModelSetResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return ModelSetResults in JSON format
    #' @export
    toJSON = function() {
      ModelSetResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        ModelSetResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        ModelSetResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        ModelSetResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        ModelSetResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        ModelSetResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      ModelSetResultsObject
    },
    #' Deserialize JSON string into an instance of ModelSetResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of ModelSetResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelSetResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[ModelSet]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to ModelSetResults
    #'
    #' @description
    #' Validate JSON input with respect to ModelSetResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ModelSetResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ModelSetResults$unlock()
#
## Below is an example to define the print function
# ModelSetResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelSetResults$lock()
