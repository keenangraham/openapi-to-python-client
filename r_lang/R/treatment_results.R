#' Create a new TreatmentResults
#'
#' @description
#' TreatmentResults Class
#'
#' @docType class
#' @title TreatmentResults
#' @description TreatmentResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Treatment}) [optional]
...
TreatmentResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new TreatmentResults class.
    #'
    #' @description
    #' Initialize a new TreatmentResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return TreatmentResults in JSON format
    #' @export
    toJSON = function() {
      TreatmentResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        TreatmentResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        TreatmentResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        TreatmentResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        TreatmentResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        TreatmentResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      TreatmentResultsObject
    },
    #' Deserialize JSON string into an instance of TreatmentResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of TreatmentResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of TreatmentResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Treatment]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to TreatmentResults
    #'
    #' @description
    #' Validate JSON input with respect to TreatmentResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of TreatmentResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# TreatmentResults$unlock()
#
## Below is an example to define the print function
# TreatmentResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# TreatmentResults$lock()
