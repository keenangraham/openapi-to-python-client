...
#' @docType class
#' @title GeneResults
#' @description GeneResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Gene}) [optional]
...
GeneResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new GeneResults class.
    #'
    #' @description
    #' Initialize a new GeneResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return GeneResults in JSON format
    #' @export
    toJSON = function() {
      GeneResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        GeneResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        GeneResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        GeneResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        GeneResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        GeneResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      GeneResultsObject
    },
    #' Deserialize JSON string into an instance of GeneResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of GeneResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of GeneResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Gene]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to GeneResults
    #'
    #' @description
    #' Validate JSON input with respect to GeneResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of GeneResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# GeneResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
