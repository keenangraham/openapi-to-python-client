#' Create a new InVitroSystemResults
#'
#' @description
#' InVitroSystemResults Class
#'
#' @docType class
#' @title InVitroSystemResults
#' @description InVitroSystemResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{InVitroSystem}) [optional]
...
#' @export
InVitroSystemResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new InVitroSystemResults class.
    #'
    #' @description
    #' Initialize a new InVitroSystemResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return InVitroSystemResults in JSON format
    #' @export
    toJSON = function() {
      InVitroSystemResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        InVitroSystemResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        InVitroSystemResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        InVitroSystemResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        InVitroSystemResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        InVitroSystemResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      InVitroSystemResultsObject
    },
    #' Deserialize JSON string into an instance of InVitroSystemResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of InVitroSystemResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of InVitroSystemResults
    #' @export
...
',
    ...
    #'
    #' @description
    #' Validate JSON input with respect to InVitroSystemResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of InVitroSystemResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# InVitroSystemResults$unlock()
#
## Below is an example to define the print function
# InVitroSystemResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# InVitroSystemResults$lock()
