#' Create a new TechnicalSampleResults
#'
#' @description
#' TechnicalSampleResults Class
#'
#' @docType class
#' @title TechnicalSampleResults
#' @description TechnicalSampleResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{TechnicalSample}) [optional]
...
#' @export
TechnicalSampleResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new TechnicalSampleResults class.
    #'
    #' @description
    #' Initialize a new TechnicalSampleResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return TechnicalSampleResults in JSON format
    #' @export
    toJSON = function() {
      TechnicalSampleResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        TechnicalSampleResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        TechnicalSampleResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        TechnicalSampleResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        TechnicalSampleResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        TechnicalSampleResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      TechnicalSampleResultsObject
    },
    #' Deserialize JSON string into an instance of TechnicalSampleResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of TechnicalSampleResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of TechnicalSampleResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[TechnicalSample]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to TechnicalSampleResults
    #'
    #' @description
    #' Validate JSON input with respect to TechnicalSampleResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of TechnicalSampleResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# TechnicalSampleResults$unlock()
#
## Below is an example to define the print function
# TechnicalSampleResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# TechnicalSampleResults$lock()
