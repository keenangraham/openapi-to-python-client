#' Create a new ImageFileResults
#'
#' @description
#' ImageFileResults Class
#'
#' @docType class
#' @title ImageFileResults
#' @description ImageFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{ImageFile}) [optional]
...
ImageFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new ImageFileResults class.
    #'
    #' @description
    #' Initialize a new ImageFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return ImageFileResults in JSON format
    #' @export
    toJSON = function() {
      ImageFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        ImageFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        ImageFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        ImageFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        ImageFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        ImageFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      ImageFileResultsObject
    },
    #' Deserialize JSON string into an instance of ImageFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of ImageFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of ImageFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[ImageFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to ImageFileResults
    #'
    #' @description
    #' Validate JSON input with respect to ImageFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ImageFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ImageFileResults$unlock()
#
## Below is an example to define the print function
# ImageFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ImageFileResults$lock()
