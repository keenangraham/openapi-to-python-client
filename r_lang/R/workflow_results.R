#' Create a new WorkflowResults
#'
#' @description
#' WorkflowResults Class
#'
#' @docType class
#' @title WorkflowResults
#' @description WorkflowResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Workflow}) [optional]
...
WorkflowResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new WorkflowResults class.
    #'
    #' @description
    #' Initialize a new WorkflowResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return WorkflowResults in JSON format
    #' @export
    toJSON = function() {
      WorkflowResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        WorkflowResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        WorkflowResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        WorkflowResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        WorkflowResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        WorkflowResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      WorkflowResultsObject
    },
    #' Deserialize JSON string into an instance of WorkflowResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of WorkflowResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of WorkflowResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Workflow]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to WorkflowResults
    #'
    #' @description
    #' Validate JSON input with respect to WorkflowResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of WorkflowResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# WorkflowResults$unlock()
#
## Below is an example to define the print function
# WorkflowResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# WorkflowResults$lock()
