#' Create a new PredictionSetResults
#'
#' @description
#' PredictionSetResults Class
#'
#' @docType class
#' @title PredictionSetResults
#' @description PredictionSetResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{PredictionSet}) [optional]
...
#' @export
PredictionSetResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new PredictionSetResults class.
    #'
    #' @description
    #' Initialize a new PredictionSetResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return PredictionSetResults in JSON format
    #' @export
    toJSON = function() {
      PredictionSetResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        PredictionSetResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        PredictionSetResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        PredictionSetResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        PredictionSetResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        PredictionSetResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      PredictionSetResultsObject
    },
    #' Deserialize JSON string into an instance of PredictionSetResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of PredictionSetResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of PredictionSetResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[PredictionSet]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to PredictionSetResults
    #'
    #' @description
    #' Validate JSON input with respect to PredictionSetResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of PredictionSetResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# PredictionSetResults$unlock()
#
## Below is an example to define the print function
# PredictionSetResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PredictionSetResults$lock()
