#' Create a new MatrixFileResults
#'
#' @description
#' MatrixFileResults Class
#'
#' @docType class
#' @title MatrixFileResults
#' @description MatrixFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{MatrixFile}) [optional]
...
MatrixFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new MatrixFileResults class.
    #'
    #' @description
    #' Initialize a new MatrixFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return MatrixFileResults in JSON format
    #' @export
    toJSON = function() {
      MatrixFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        MatrixFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        MatrixFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        MatrixFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        MatrixFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        MatrixFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      MatrixFileResultsObject
    },
    #' Deserialize JSON string into an instance of MatrixFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of MatrixFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of MatrixFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[MatrixFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to MatrixFileResults
    #'
    #' @description
    #' Validate JSON input with respect to MatrixFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of MatrixFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# MatrixFileResults$unlock()
#
## Below is an example to define the print function
# MatrixFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# MatrixFileResults$lock()
