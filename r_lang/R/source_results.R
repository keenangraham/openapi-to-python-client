...
#' @docType class
#' @title SourceResults
#' @description SourceResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Source}) [optional]
...
SourceResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SourceResults class.
    #'
    #' @description
    #' Initialize a new SourceResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SourceResults in JSON format
    #' @export
    toJSON = function() {
      SourceResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SourceResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SourceResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SourceResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SourceResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SourceResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SourceResultsObject
    },
    #' Deserialize JSON string into an instance of SourceResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SourceResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SourceResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Source]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SourceResults
    #'
    #' @description
    #' Validate JSON input with respect to SourceResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SourceResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SourceResults$unlock()
#
## Below is an example to define the print function
# SourceResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
