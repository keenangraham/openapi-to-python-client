#' Create a new SoftwareResults
#'
#' @description
#' SoftwareResults Class
#'
#' @docType class
#' @title SoftwareResults
#' @description SoftwareResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{Software}) [optional]
...
SoftwareResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SoftwareResults class.
    #'
    #' @description
    #' Initialize a new SoftwareResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SoftwareResults in JSON format
    #' @export
    toJSON = function() {
      SoftwareResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SoftwareResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SoftwareResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SoftwareResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SoftwareResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SoftwareResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SoftwareResultsObject
    },
    #' Deserialize JSON string into an instance of SoftwareResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SoftwareResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SoftwareResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[Software]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SoftwareResults
    #'
    #' @description
    #' Validate JSON input with respect to SoftwareResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SoftwareResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SoftwareResults$unlock()
#
## Below is an example to define the print function
# SoftwareResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# SoftwareResults$lock()
