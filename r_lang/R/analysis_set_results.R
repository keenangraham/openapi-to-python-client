#' Create a new AnalysisSetResults
#'
#' @description
#' AnalysisSetResults Class
#'
#' @docType class
#' @title AnalysisSetResults
#' @description AnalysisSetResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{AnalysisSet}) [optional]
...
AnalysisSetResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new AnalysisSetResults class.
    #'
    #' @description
    #' Initialize a new AnalysisSetResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return AnalysisSetResults in JSON format
    #' @export
    toJSON = function() {
      AnalysisSetResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        AnalysisSetResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        AnalysisSetResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        AnalysisSetResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        AnalysisSetResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        AnalysisSetResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      AnalysisSetResultsObject
    },
    #' Deserialize JSON string into an instance of AnalysisSetResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of AnalysisSetResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of AnalysisSetResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[AnalysisSet]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to AnalysisSetResults
    #'
    #' @description
    #' Validate JSON input with respect to AnalysisSetResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of AnalysisSetResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# AnalysisSetResults$unlock()
#
## Below is an example to define the print function
# AnalysisSetResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AnalysisSetResults$lock()
