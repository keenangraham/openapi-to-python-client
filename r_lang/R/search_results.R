...
#' @docType class
#' @title SearchResults
#' @description SearchResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{SearchResultItem}) [optional]
...
SearchResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SearchResults class.
    #'
    #' @description
    #' Initialize a new SearchResults class.
    #'
    ...
    #' @export
    toJSON = function() {
      SearchResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SearchResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SearchResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SearchResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SearchResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SearchResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SearchResultsObject
    },
    #' Deserialize JSON string into an instance of SearchResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SearchResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SearchResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[SearchResultItem]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SearchResults
    #'
    #' @description
    #' Validate JSON input with respect to SearchResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SearchResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SearchResults$unlock()
#
## Below is an example to define the print function
# SearchResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
