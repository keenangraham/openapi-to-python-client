#' Create a new ModelFileResults
#'
#' @description
#' ModelFileResults Class
#'
#' @docType class
#' @title ModelFileResults
#' @description ModelFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{ModelFile}) [optional]
...
ModelFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new ModelFileResults class.
    #'
    #' @description
    #' Initialize a new ModelFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return ModelFileResults in JSON format
    #' @export
    toJSON = function() {
      ModelFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        ModelFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        ModelFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        ModelFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        ModelFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        ModelFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      ModelFileResultsObject
    },
    #' Deserialize JSON string into an instance of ModelFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of ModelFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[ModelFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to ModelFileResults
    #'
    #' @description
    #' Validate JSON input with respect to ModelFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ModelFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ModelFileResults$unlock()
#
## Below is an example to define the print function
# ModelFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelFileResults$lock()
