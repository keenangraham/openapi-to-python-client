#' Create a new ConfigurationFileResults
#'
#' @description
#' ConfigurationFileResults Class
#'
#' @docType class
#' @title ConfigurationFileResults
#' @description ConfigurationFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{ConfigurationFile}) [optional]
...
#' @export
ConfigurationFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new ConfigurationFileResults class.
    #'
    #' @description
    #' Initialize a new ConfigurationFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return ConfigurationFileResults in JSON format
    #' @export
    toJSON = function() {
      ConfigurationFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        ConfigurationFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        ConfigurationFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        ConfigurationFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        ConfigurationFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        ConfigurationFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      ConfigurationFileResultsObject
    },
    #' Deserialize JSON string into an instance of ConfigurationFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of ConfigurationFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of ConfigurationFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[ConfigurationFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to ConfigurationFileResults
    #'
    #' @description
    #' Validate JSON input with respect to ConfigurationFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ConfigurationFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ConfigurationFileResults$unlock()
#
## Below is an example to define the print function
# ConfigurationFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ConfigurationFileResults$lock()
