#' Create a new SignalFileResults
#'
#' @description
#' SignalFileResults Class
#'
#' @docType class
#' @title SignalFileResults
#' @description SignalFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{SignalFile}) [optional]
...
SignalFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new SignalFileResults class.
    #'
    #' @description
    #' Initialize a new SignalFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return SignalFileResults in JSON format
    #' @export
    toJSON = function() {
      SignalFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        SignalFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        SignalFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        SignalFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        SignalFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        SignalFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      SignalFileResultsObject
    },
    #' Deserialize JSON string into an instance of SignalFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of SignalFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of SignalFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[SignalFile]", loadNamespace("igvf_client"))
      }
...
',
    ...
    #' Validate JSON input with respect to SignalFileResults
    #'
    #' @description
    #' Validate JSON input with respect to SignalFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of SignalFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# SignalFileResults$unlock()
#
## Below is an example to define the print function
# SignalFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# SignalFileResults$lock()
