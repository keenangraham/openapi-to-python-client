...
MEASUREMENTSETCONTROLFILESETSINNER_ONE_OF_SCHEMAS = ["AnalysisSet", "AuxiliarySet", "ConstructLibrarySet", "CuratedSet", "MeasurementSet", "ModelSet", "PredictionSet", "str"]

class MeasurementSetControlFileSetsInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MeasurementSetControlFileSetsInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MeasurementSetControlFileSetsInner with oneOf schemas: AnalysisSet, AuxiliarySet, ConstructLibrarySet, CuratedSet, MeasurementSet, ModelSet, PredictionSet, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MeasurementSetControlFileSetsInner with oneOf schemas: AnalysisSet, AuxiliarySet, ConstructLibrarySet, CuratedSet, MeasurementSet, ModelSet, PredictionSet, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MeasurementSetControlFileSetsInner with oneOf schemas: AnalysisSet, AuxiliarySet, ConstructLibrarySet, CuratedSet, MeasurementSet, ModelSet, PredictionSet, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MeasurementSetControlFileSetsInner with oneOf schemas: AnalysisSet, AuxiliarySet, ConstructLibrarySet, CuratedSet, MeasurementSet, ModelSet, PredictionSet, str. Details: " + ", ".join(error_messages))
        else:
            return instance
...
MeasurementSetControlFileSetsInner.model_rebuild(raise_errors=False)
