...
class AlignmentFile(BaseModel):
    ...
    schema_version: Optional[Annotated[str, Field(strict=True)]] = Field(default='10', description="The version of the JSON schema that the server uses to validate the object.")
    ...
    redacted: StrictBool = Field(description="Indicates whether the alignments data have been sanitized (redacted) to prevent leakage of private and potentially identifying genomic information.")
    filtered: StrictBool = Field(description="Indicates whether reads that did not pass a filtering step, such as PCR duplicates, have been removed from the file.")
    ...
    def content_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['alignments', 'transcriptome alignments']):
            raise ValueError("must be one of enum values ('alignments', 'transcriptome alignments')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "schema_version": obj.get("schema_version") if obj.get("schema_version") is not None else '10',
            "uuid": obj.get("uuid"),
            "notes": obj.get("notes"),
