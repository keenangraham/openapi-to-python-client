...
class GenomeBrowserAnnotationFile(BaseModel):
    ...
    derived_from: Annotated[List[AlignmentFileDerivedFromInner], Field(min_length=1)] = Field(description="The files participating as inputs into software to produce this output file.")
    ...
    def content_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['peaks']):
            raise ValueError("must be one of enum values ('peaks')")
        return value

    @field_validator('file_format')
    def file_format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['bigBed', 'tabix']):
            raise ValueError("must be one of enum values ('bigBed', 'tabix')")
        return value
...
GenomeBrowserAnnotationFile.model_rebuild(raise_errors=False)
