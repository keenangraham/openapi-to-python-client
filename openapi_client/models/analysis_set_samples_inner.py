...
ANALYSISSETSAMPLESINNER_ONE_OF_SCHEMAS = ["InVitroSystem", "MultiplexedSample", "PrimaryCell", "TechnicalSample", "Tissue", "WholeOrganism", "str"]

class AnalysisSetSamplesInner(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AnalysisSetSamplesInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AnalysisSetSamplesInner with oneOf schemas: InVitroSystem, MultiplexedSample, PrimaryCell, TechnicalSample, Tissue, WholeOrganism, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AnalysisSetSamplesInner with oneOf schemas: InVitroSystem, MultiplexedSample, PrimaryCell, TechnicalSample, Tissue, WholeOrganism, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AnalysisSetSamplesInner with oneOf schemas: InVitroSystem, MultiplexedSample, PrimaryCell, TechnicalSample, Tissue, WholeOrganism, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AnalysisSetSamplesInner with oneOf schemas: InVitroSystem, MultiplexedSample, PrimaryCell, TechnicalSample, Tissue, WholeOrganism, str. Details: " + ", ".join(error_messages))
        else:
            return instance
...
AnalysisSetSamplesInner.model_rebuild(raise_errors=False)
