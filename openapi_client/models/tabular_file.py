...
class TabularFile(BaseModel):
    ...
    schema_version: Optional[Annotated[str, Field(strict=True)]] = Field(default='11', description="The version of the JSON schema that the server uses to validate the object.")
    ...
    def content_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['barcode to element mapping', 'barcode to sample mapping', 'differential gene expression quantifications', 'differential transcript expression quantifications', 'element quantifications', 'elements reference', 'fold change over control', 'guide quantifications', 'guide RNA sequences', 'MPRA sequence designs', 'peaks', 'protein to protein interaction score', 'sample sort parameters', 'sequence barcodes', 'variant effects', 'variant to element mapping']):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "schema_version": obj.get("schema_version") if obj.get("schema_version") is not None else '11',
            "uuid": obj.get("uuid"),
            "notes": obj.get("notes"),
...
TabularFile.model_rebuild(raise_errors=False)
