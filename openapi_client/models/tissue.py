...
class Tissue(BaseModel):
    ...
    pooled_from: Optional[Annotated[List[TissuePooledFrom], Field(min_length=2)]] = Field(default=None, description="The biosamples this biosample is pooled from.")
    part_of: Optional[PartOfBiosample2] = None
    ...
    pmi: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="The amount of time elapsed since death.")
    pmi_units: Optional[StrictStr] = Field(default=None, description="The unit in which the PMI time was reported.")
    ccf_id: Optional[StrictStr] = Field(default=None, description="HubMap Common Coordinate Framework unique identifier corresponding to the organ, biological structure, and spatial location of the tissue specimen.")
    preservation_method: Optional[StrictStr] = Field(default=None, description="The method by which the tissue was preserved: cryopreservation (slow-freeze) or flash-freezing.")
    ...
    @field_validator('pmi_units')
    def pmi_units_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['second', 'minute', 'hour', 'day', 'week']):
            raise ValueError("must be one of enum values ('second', 'minute', 'hour', 'day', 'week')")
        return value

    @field_validator('preservation_method')
    def preservation_method_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['cryopreservation', 'flash-freezing']):
            raise ValueError("must be one of enum values ('cryopreservation', 'flash-freezing')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "pooled_from": [TissuePooledFrom.from_dict(_item) for _item in obj["pooled_from"]] if obj.get("pooled_from") is not None else None,
            "part_of": PartOfBiosample2.from_dict(obj["part_of"]) if obj.get("part_of") is not None else None,
            ...
            "pmi": obj.get("pmi"),
            "pmi_units": obj.get("pmi_units"),
            "ccf_id": obj.get("ccf_id"),
            "preservation_method": obj.get("preservation_method"),
            "@id": obj.get("@id"),
            "@type": obj.get("@type"),
...
Tissue.model_rebuild(raise_errors=False)
