...
LINKTO16_ONE_OF_SCHEMAS = ["AlignmentFile", "ConfigurationFile", "GenomeBrowserAnnotationFile", "ImageFile", "MatrixFile", "ModelFile", "ReferenceFile", "SequenceFile", "SignalFile", "TabularFile", "str"]

class LINKTO16(BaseModel):
    """
    LINKTO16
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = LINKTO16.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in LINKTO16 with oneOf schemas: AlignmentFile, ConfigurationFile, GenomeBrowserAnnotationFile, ImageFile, MatrixFile, ModelFile, ReferenceFile, SequenceFile, SignalFile, TabularFile, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in LINKTO16 with oneOf schemas: AlignmentFile, ConfigurationFile, GenomeBrowserAnnotationFile, ImageFile, MatrixFile, ModelFile, ReferenceFile, SequenceFile, SignalFile, TabularFile, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into LINKTO16 with oneOf schemas: AlignmentFile, ConfigurationFile, GenomeBrowserAnnotationFile, ImageFile, MatrixFile, ModelFile, ReferenceFile, SequenceFile, SignalFile, TabularFile, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into LINKTO16 with oneOf schemas: AlignmentFile, ConfigurationFile, GenomeBrowserAnnotationFile, ImageFile, MatrixFile, ModelFile, ReferenceFile, SequenceFile, SignalFile, TabularFile, str. Details: " + ", ".join(error_messages))
        else:
            return instance
...
LINKTO16.model_rebuild(raise_errors=False)
